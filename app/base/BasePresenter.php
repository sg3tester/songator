...
abstract class BasePresenter extends Nesys\NesysPresenter
{
	...
	if ($this->getHttpRequest()->getCookie('isEnabledCookies') != true) 
	{ 
	  if ($this->getHttpRequest()->getQuery("isEnabledCookies") == "test")
	  {
		echo "Pro vstup na tento portál musíte mít povoleny cookies";
		$this->core->log(\Nesys\NesysCore::LOG_ERROR, "Neplatný přístup z ".$_SERVER['REMOTE_ADDR']." do ".$_SERVER['REQUEST_URI']." (pravděpodobně bot)");
		$this->terminate();
	  }
	  else 
	  {
		$this->getHttpResponse()->setCookie('isEnabledCookies', true, 0); 
		$this->redirect("this", array("isEnabledCookies" => "test"));
	  }
	}
	/*
